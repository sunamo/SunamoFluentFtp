...
public class FluentFtpWrapper : FtpBaseNew
{
    ...
    {
        if (!slozkaNaHostingu.StartsWith(AllStringsSE.slash))
        {
            slozkaNaHostingu = AllStringsSE.slash + slozkaNaHostingu;
        }
...
await
#endif
SunamoExceptions.TFSE.ReadAllBytesArray(path), FS.GetFileName(path), FtpRemoteExists.Overwrite);
    }
